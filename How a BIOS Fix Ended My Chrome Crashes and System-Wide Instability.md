...
The "Magic" Stability Blueprint:
    ...
    Switch to Wayland: (For NVIDIA users) It handles shared memory better than X11 and stops the "Sandbox" hangs.

    Disable Intel C-States: In the BIOS, set C-States to [Disabled]. This stops the violent voltage "surges" that occur when the CPU wakes up from sleep.

    Lock Power Limits (PL1/PL2): Set both to 253W. Do not leave them on "Unlimited."

    Limit Current (ICCMax): Set this to 280A or 307A to prevent electrical spikes.
